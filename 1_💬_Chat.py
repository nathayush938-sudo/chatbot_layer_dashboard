...
def fmt_pct_smart(value, decimals=1) -> str:
    """
    Format a percentage regardless of whether it's a decimal ratio (0.748)
    or already-multiplied (74.8). Detects by magnitude ≤1.5 → decimal.
    """
...
def is_metric_column(col):
    col_lower = col.lower()
    # Group-prefixed KPI columns (total__, bucket__, cjs__, etc.) are always metrics
    if any(col_lower.startswith(p) for p in ("total__", "bucket__", "cjs__", "region__")):
        return True
    # Columns ending in _usd or _inr are always metrics
    if col_lower.endswith("_usd") or col_lower.endswith("_inr"):
        return True
...
def apply_display_formatting(df, metadata):
    ...
    if metric_candidates:
        ...
        if dim_cols:
            ...
            if is_ageing:
                _ar_specific = {'Current', '1-30 days', '31-60 days', '91-180 days', '>180 days'}
                active_order = AR_AGEING_ORDER if any(v in _ar_specific for v in sample_vals) else AGEING_ORDER
...
def apply_ageing_pivot_formatting(pivot_df, metadata, row_cols):
    ...
    # Use AR-specific values to detect AR (not '61-90 days' which appears in both)
    AR_SPECIFIC = {'Current', '1-30 days', '31-60 days', '91-180 days', '>180 days'}
    if any(c in AR_SPECIFIC for c in pivot_df.columns):
    ...
    for col in ageing_cols:
        gt[col] = format_currency_value(float(pivot_df[col].sum()), currency)
    gt['Row Total'] = format_currency_value(grand_total, currency)
    gt['Row %']     = "100.0%"
    result_rows.append(gt)
...
def render_result(df, metadata, result, view_key: str = "default"):
    ...
    elif (visualization == "pivot_table"
          and metadata.get("rows")
          and metadata.get("columns")):
        # Dimension pivot: tall data (ageingbucket, quarter as cols) → frontend pivots
        metadata["pivot_type"] = "dimension"
    ...
    try:
        if visualization == "pivot_table" and metadata.get("pivot_type") == "metric":
            ...
            # AR-specific buckets — don't overlap with collections ('61-90 days' is in both)
            _ar_spec             = {'Current', '1-30 days', '31-60 days', '91-180 days', '>180 days'}
            ar_ageing_in_pivot   = [c for c in AR_AGEING_ORDER
                                    if c in pivot_df.columns and c in _ar_spec]
            ...
            if is_detail and len(df) > 1:
                ...
                if metric_cols:
                    # Detect prefix groups: total__, bucket__, cjs__, etc.
                    GROUP_LABELS = {
                        "total__":  "Overview",
                        "bucket__": "Outstanding by Client Bucket",
                        "cjs__":    "Outstanding by Journey Stage",
                    }
                    groups = {}
                    ungrouped = []
                    ...
                    def _is_pct_col(name: str) -> bool:
                        ...
                        return any(kw in n for kw in
                                   ["%", "pct", "percent", "ratio", "rate", "share", "efficiency"])

                    def _render_kpi_row(cols, str_col=None):
                        """Render KPI cards. str_col shown as first card if provided."""
                        all_cols = ([str_col] if str_col else []) + list(cols)
                        kpi_cols = st.columns(len(all_cols))
                        for i, col in enumerate(all_cols):
                            label = strip_currency_suffix(col_map.get(col, col))
                            if col == str_col:
                                val = str(df[col].iloc[0]) if pd.notnull(df[col].iloc[0]) else "—"
                                kpi_cols[i].metric(label=label, value=val)
                            else:
                                raw = df[col].iloc[0]
                                if pd.isna(raw):
                                    value = "—"
                                elif _is_pct_col(label) or _is_pct_col(col):
                                    value = fmt_pct_smart(float(raw))
                                else:
                                    value = format_currency_value(float(raw), currency)
                                kpi_cols[i].metric(label=label, value=value)

                    # First string column becomes its own card (e.g. entity, customer, region)
                    string_cols = df.select_dtypes(include=["object", "str"]).columns.tolist()
                    str_col = string_cols[0] if string_cols else None

                    if groups:
                        # Render each prefix group as a labeled section
                        for prefix, label in GROUP_LABELS.items():
                            if prefix in groups:
                                st.caption(f"**{label}**")
                                _render_kpi_row(groups[prefix])
                        if ungrouped:
                            _render_kpi_row(ungrouped, str_col=str_col)
                    else:
                        # No prefix groups — str_col on first row, then metric rows of 4
                        MAX_PER_ROW = 4
                        rows = [metric_cols[i:i+MAX_PER_ROW]
                                for i in range(0, len(metric_cols), MAX_PER_ROW)]
                        for idx_r, row_cols in enumerate(rows):
                            _render_kpi_row(row_cols,
                                            str_col=str_col if idx_r == 0 else None)
                else:
